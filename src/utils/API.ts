...
import * as akatsuki from './akatsuki';
...
export async function getUser(userID : string, ruleset = 0, serverType = 0, relax = false, isLaser = false) : Promise<Array<IAPIUser>> {
	return ( await axios.get(`https://${isLaser ? "lazer": "osu"}.ppy.sh/api/get_user?k=${process.env.osuAPI}&u=${userID}&m=${ruleset}`)).data
