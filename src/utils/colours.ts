...
export function getColorBlack(color) {
	color = toRGB(color)
	const red = color[0];
	const green = color[1];
	const blue = color[2];
...
export async function getColours(url: string, legacyCampability: Boolean, callback: (colours: { foreground: string, background: string }) => void = (): void => { }): Promise<void> {
	if (url === undefined) {
		const template = Buffer.from(fs.readFileSync(path.resolve(__dirname, '../assets/unknown_bg.png')));
		const { dominant } = await sharp(template).stats();
		...
		Vibrant.from(template).maxColorCount(64).getPalette(async function (err: any, palette: any) {

			
			let backgroundColour = toHex(dm);
